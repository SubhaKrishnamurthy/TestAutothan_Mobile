...
public class PropertyReader {
    ...
    public static By locatorOf(String locatorKey) {
        ...
        locatorType.add("IOSCLASSCHAIN");

        File file = new File(Test.attributes.get(Keys.ObjectRepository));
        File file1 = new File(Test.attributes.get(Keys.ObjectRepository1));
        Properties prop = new Properties();
        ...
        try {
            ...
            switch (locatorSplit[locatorSplit.length - 1].trim().toUpperCase()) {
                ...
                case "CLASS":
                    ...
                    break;
                case "IOSCLASSCHAIN":
                    locatorBy = MobileBy.iOSClassChain(locator);
                    break;
                default:
                    locatorBy = By.xpath(locator);
            }
    ...
    public static String dynamicReadTestDataOf(String valueOf) {
        File file = new File("src/test/resources/data/Dynamic-Data-Input-"+Test.attributes.get(Keys.OS)+".properties");
        Properties prop = new Properties();
        InputStream input = null;
        try {
            input = new FileInputStream(file);
    ...
    public static void dynamicWriteTestDataOf(String Key, String valueToBeChanged) {
        File file = new File("src/test/resources/data/Dynamic-Data-Input-"+Test.attributes.get(Keys.OS)+".properties");
        Properties prop = new Properties();
        try {
            ...
            prop.replace(Key,valueToBeChanged);
            prop.store(outputStream, "updated value for " + Key);
            outputStream.close();
        } catch (Exception ex) {
